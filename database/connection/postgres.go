...
func (e EmployeeRepository) CreateEmployee(ctx context.Context, dto dto.EmployeeDTO) (*database.Employee, error) {
	...
	if err != nil {
		...
		if errors.As(err, &e) && e.Code == NAME_ALREADY_EXIST {
			return nil, db_err.ErrDuplicate
		}
		return nil, err
	}
	return &emp, err
...
func (e EmployeeRepository) UpdateEmployee(ctx context.Context, dto dto.EmployeeDTO, id int64) (*database.Employee, error) {
	...
	qtx := e.WithTx(tx)
	queriedEmp, err := qtx.GetEmployee(ctx, id)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, db_err.ErrNotFound
		}
	...
	parsedEmp := utils.RemoveEmptyDataToArgs(dto, *utils.ParseSingleEmployee(&queriedEmp))

	parsedEmp.ID = id
	employee, err := qtx.UpdateEmployee(ctx, parsedEmp)
	if err != nil {
		return nil, err
	}
	tx.Commit(ctx)
